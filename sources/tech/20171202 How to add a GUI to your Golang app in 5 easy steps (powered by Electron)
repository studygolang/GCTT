# 仅需简单5步，给你的Golang程序添加GUI

------

创建一个Golang app是一件简单又轻松的事情，但是有时候你想给你的应用锦上添花：创建一个GUI！

在本篇文章中，我将通过使用astilectron工具中的bootstrap以及bundler给一个简单的Golang程序添加GUI
我们的带有GUI的Golang app能够打开一个文件夹并且展示其中的内容。
...
##第二步：搭建基础架构
首先我们需要在 `main.go` 中导入[astilectron](https://github.com/asticode/go-astilectron)的bootstrap源码包
...
我们也要在go上添加 `debug` 选项，因为我们需要使用HTML/JS/CSS/调试工具

最后我们将全局变量指针w存入 `astilectron.Window` ，因为我们需要这个指针在我们使用 `OnWait` 时包含一个回调执行当窗口，菜单和所有的对象将创建时。


##HTML
现在我们需要在 `resources/app/index.html` 中创建我们的HTML主页：
```html
<!DOCTYPE html>
<html lang="en">
<head>
...
现在我们需要在 `resources/app/static/css/base.css` 文件中创建我们的CSS：
```css
...
```
##JS
然后我们在 `resources/app/static/js/index.js` 中创建JS：
```javascript
let index = {
    init: function() {
        // Init
...
## 第三步：建立起GO与Javascript间的连接
万事俱备，只欠东风：我们需要将GO和Javascript连接起来

###Javascript连接GO
为了让Javascript连接GO，首先从Javascript向GO发送一条消息，并且在GO接受到消息后执行回调函数
```javascript
...
```
同时我们在GO中监听来自Javascript的消息，并且通过bootstrap的 `MessageHandler` 给Javascript发送消息
```go
func main() {
...
```
一旦Javascript的 `astilectron` 命名空间准备好，它执行新的 `explore` 方法，该方法会给GO发送一条消息，接收返回的信息，并且更新相应的HTML
...
为了建立从GO到Javascript的连接，我们首先需要从GO中向Javascript发送一条消息并且在Javascript收到消息后执行回调。
```javascript
...
同时我们在Javascript中监听来自GO的消息并发送一个选项消息给GO：
...
这个简单的例子将在GO的输出中打印 `收到的HelloWorld` 。在我们的项目里，我们先将下面的代码加入到 `main.go` 中：
```
func main() {
	bootstrap.Run(bootstrap.Options{
		MenuOptions: []*astilectron.MenuItemOptions{{
...
它使得关于选项变成可点击的，并且渲染出一个有合适内容的模态框，在GO app完成初始话后它会显示一个5秒钟的提示框。
...
它将监听GO发送过来的消息并做出响应的反应。

#第四步: 打包到app

现在代码已经完成，我们需要确保我们能够以最好的方式把我们的 Golang GUI app呈现给我们的用户：

 - 一个MacOSX app给 `darwin` 用户
 - 一个含有好看图标的 `.exe` 给 `windows` 用户
 - 一个简单的源码文件给 `linux` 用户


幸运的是，我们可以通过astilectron的bundler来进行操作。
首先我们通过下面命令进行安装：
```
...
然后我们在 `main.go` 中给bootstrap添加配置项：
```
func main() {
	bootstrap.Run(bootstrap.Options{
		Asset: Asset,
...
![](http://p0kkk2bd3.bkt.clouddn.com/0.png)
你当然可以打包你的Golang GUI app 给其他环境，在bundler打包文档中查看如何将程序打包到其他环境
#结论
感谢 astilectron的bootstrap和bundler，通过一点对组织结构变动工作，给你的Golang 程序添加GUI从未如此简单。
需要指出的是这种方法有2个主要的缺点：
- 代码包的大小至少有50MB，第一次执行后，文件大小至少将有200MB
- 内存的消耗有些疯狂，因为Electron并不擅长对内存的管理


但是如果你准备掌握它，那么你在给你的程序添加GUI时将非常便利。
GUI编码愉快
