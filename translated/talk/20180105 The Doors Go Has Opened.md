...
![image](https://raw.githubusercontent.com/ashleymcnamara/gophers/master/BATMAN_GOPHER.png)
...
- [etcd](https://github.com/coreos/etcd) ：一个健壮的高一致性的 key-value 数据库。它被用在分布式系统中处理核心任务，实际上它就成为了和谷歌 Chubby （译者注：一个分布式锁服务） 一样的开源软件。它是 Kubernetes 核心组件。
- [Docker](http://docker.com/) ：它是目前市面上非常流行的容器引擎。也是 Kubernetes 的核心组件。


Cloud Native 将不可能抛弃 Go ，[Cloud Native Computing Foundation](https://www.cncf.io) （CNCF基金会）同样也不会。这仅仅是个开始。Go 也接管了其它部分开源软件，更别提那些大公司内部的的基础设施。

实际上这就是为什么大家都在选择开源（或者其它的方式）用 Go 创建或者重构的产品系统和大型软件。
...
### *无论你在做什么，你都要关注你做的。Go 已经为你找到了解决问题的工具*

然后你去下载一个 toolchain （译者注：工具链，一般指的就是编译工具）—— 一个二进制 `go` 文件。你可以通过运行 `go build` 来获一个生产级别的软件。无需学习 GCC toolchain ，C 语言，Linux ，共享对象，JVM 或其它相关技术。  

不管你再开发什么，你都要专注开发的业务，而不是你需要哪些工具。Go已经为你解决了相应的工具了。

## 二进制文件

和公司的上年纪的开发工作时，你编译代码时，不能仅仅只是运行它。你需要在系统上运行其它的东西：共享对象，JVM，等等。


### *仅需要传送一个二进制文件到服务器即可*

`go build` 会输出一个可执行的二进制文件。将它发送到你的服务器上。它之所以能运行你因为已经将所需要的东西都编译进去了。
这个简单的案例展示了它的强大。最好的事情是你的部署过程将比以前简单的多。—— 仅需要将二进制文件传送到你的服务器即可。
你甚至可以通过少量的环境变量在不同的系统上构建。这个特性非常适合 CLIs （译者注：命令行执行）一下是最成功的几个特性：
- `tecdctl` : etcd 下的 CLI
...
和上了年纪的开发一起工作时，你不得不去关注系统线程，锁，条件变量（译者注：多线程开发名词）等等。为了达到一个高效的目的，需要系统有弹性的学习能力 —— 或创新能力 —— 一个高并发框架然后你就可以继续了。

现在你可以获得一个简单易懂的内置基本操作。 Goroutines 和 channels 是有意义的，应为它模仿是的真实的情况。


你只需要将 `go` 关键字放到一个方法前面它最终就是以并发的方式运行的。你可以很容易的理解这些并发功能，并且可以专注你的业务开发。是否看到了一个趋势？
...
手动管理内存是很困难的，当你有高并发需求时更难。在 Go 以前，我们将面临许多冲突与挑战：我们需要一个不会让你程序内存泄漏或崩溃的框架，但那对程序员来说是有意义的。
...
事实上，GC 已经爆式的促进了 Go 。下面这些是 Go 垃圾回收的边界情况，可能会出现一些问题。但是很多[看法](https://docs.google.com/document/d/16Y4IsnNRCN43Mx0NZc5YXZLovrHvvLhK_h0KN8woTO4/edit)都是为了让它更好的运行，默认 90% 是这样。
...
Go 标准库是最好的商业库之一。它不大但是却覆盖了 80% 的常用功能，并且不复杂却可以为你完成复杂是事情。
...
-  [erros](https://godoc.org/builtin#error) 是之中从函数中返回错误描述的方法
...
我希望 Go 继续成为其它领域的标准——前端服务（替代 Rails / Node
.js），CLIs （替换许多脚本语言），也许也能替换 GUIs 和 移动 APP 。

正值 Go [8 周年](https://blog.golang.org/8years) ，它快速的崛起了。但下一个 8 年它的趋势是否会扩大10倍。
