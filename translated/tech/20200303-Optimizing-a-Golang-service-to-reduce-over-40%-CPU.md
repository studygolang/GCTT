...
在 Coralogix（译者注：一个提供全面日志分析的服务产品，[官网](https://coralogix.com/)），我们为了去给我们的客户提供关于他们日志实时的分析，警报和元数据，要去解析他们的日志。为了去做到这一点，在解析阶段，也是非常复杂并且负载了每行日志服务的许多规则，必须非常快速。这就是我们决定去使用 Golang 的原因之一。
...
因此，内存消耗**由大约 800 MB 提高到了 仅 180 MB**。
...
WithLabelValues 的两个调用都是为了软件度量的普罗米修斯函数（我们将这个留给 Product 去决定是否真正需要）。而且，我们可以看到第一行创建了大量的对象（由这个函数所创建的全部对象的 10%）。我们进一步查看发现它是一个对于绑定到导出数据的消费者 ID 从 int 到 string 的转换，十分重要，但是考虑到实际情况，我们数据库中消费者的数量十分有限，我们不应该采用普罗米修斯的方式来接收变量作为 string 类型（译者注：意思是不必要为了做到这一点而牺牲整个服务的性能）。因此取代了每次创建一个新的 string 并且在函数末尾都抛弃的这种方法（浪费分配还有 GC 的多余工作），我们在对象的分配阶段定义了 map，配对了所有从 1 到 10 万的数字和一个需要执行的 “get” 方法。
