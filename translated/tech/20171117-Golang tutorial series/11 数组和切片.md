...
一个数组的表示形式为 `n[T]`。`n` 表示数组中元素的数量，`T` 代表每个元素的类型。元素 `n` 的个数也是该类型的一部分（稍后我们将详细讨论这一点）。
...
Go 中的数组是值类型而不是引用类型。 这意味着当它们被分配给一个新的变量时，原始数组的一个副本被分配给新的变量。 如果对新变量进行更改，则不会影响原始数组。
...
Go 提供了一种更好、更简洁的方法，通过使用 `for` 循环的 ** range ** 形式来遍历数组。`range` 返回索引和该索引处的值。让我们使用 range 重写上面的代码。我们还可以获取数组中所有元素的总和。
...
到目前为止我们创建的数组都是单维的，Go 语言可以创建多维数组。
...
在上述程序的第 17 行，一个二维字符串数组 a 用简略语法声明。20 行末尾的逗号是必需的。这是因为根据 Go 语言的规则自动插入分号。如果你有兴趣了解更多关于为什么这是必需的，请阅读 [https://golang.org/doc/effective_go.html#semicolons](https://golang.org/doc/effective_go.html#semicolons)。
...
这就是数组,尽管数组看上去似乎足够灵活，但是它们具有固定长度的限制。不可能增加数组的长度。这就是 ** 切片 ** 进入场景。事实上，在 Go 中，切片比传统数组更常见。

## 切片

切片是一种方便、灵活且功能强大的数组顶层包装。切片本身不拥有任何数据。它们只是对现有数组的引用。
...
在上述程序的第 9 行，`c：= [] int {6，7，8}` 创建一个有 3 个整形元素的数组，并返回一个存储在 c 中的切片引用。
...
当多个切片共用相同的基础数组时，每个切片所做的更改将反映在数组中。
...
在上述程序的 11 行中，`fruitslice` 的容量是重置的。以上程序输出为，
...
func make（[]T，len，cap）[]T 通过传递类型，长度和容量来创建切片。容量参数是可选的, 默认为切片长度。make 函数创建一个数组并返回对它切片的引用。
...
** x ... T ** 在函数定义中表示该函数接受参数 x 是可变数量的。这些类型的函数被称为[可变函数](https://golangbot.com/variadic-functions/)。
...
在上述程序中，`cars` 的容量最初是 3。我们在第 10 行添加了一个新的元素分配给切片，其返回为 `append(cars, "Toyota")` 之后 cars 的值。现在 cars 的容量翻了一番，变成了 6。上述程序的输出是
...
在上面的程序 `names` 是零，我们已经添加 3 个字符串给 `names`。该程序的输出是
...
切片持有对底层数组的引用。只要切片在内存中，数组就不能被垃圾回收。在内存管理方面，这是需要注意的。让我们假设我们有一个非常大的数组，我们只想处理它的一小部分。然后，我们从该阵列创建一个切片，并开始处理切片。这里需要重点注意的是，在切片引用时数组仍然存在内存中。

解决此问题的一种方法是使用[复制](https://golang.org/pkg/builtin/#copy)函数 `func copy(dst，src[]T)int` 来复制该切片。这样我们可以使用新的切片，原始数组可以被垃圾回收。
