...
正如你所见的（不出所料），对于不同的“n”，Map 查找是常数复杂度（O(1)）。然而，有趣的是，对于“n”较小的 Slice，简单的 _for 循环和 if _比较花费的时间比 Map 少。较大的“n”如期望那样花费更多的时间。
...
**结论：直接查找是常数复杂度，所以，假定你知道索引，那么使用哪个都无所谓。但是，假定索引已知，那么 Slice 或者数组查找仍然比 Map 查找快得多。**
