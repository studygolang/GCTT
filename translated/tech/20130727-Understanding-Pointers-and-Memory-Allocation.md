...
在 Go 中，我们可以创建一个变量作为"值"本身，也可以创建一个变量作为某个"值"的地址。当变量的“值”是地址时，我们就称该变量为地址。
在下图中，有一个名为 myVariable 的变量。 myVariable 的“值”是指向另一个值的地址。则 myVariable 被叫做指针变量。
...
仔细观察第二三行的输出结果，为什么会这样？在函数调用之后，main 函数的 myValue 的值并没有得到修改。因为 main 中的 myValue 变量的“值”在被函数调用时只是被复制了一份，并不包含对这个值的引用操作，它不是指针而只是一个单纯的值。当我们将 main 中的 myValue 变量的“值”传递给函数时，该值被拷贝后将放置在堆栈上。函数改变的是只是 myValue 拷贝后的副本。一旦函数终止，副本就会弹出堆栈，并且在技术栈上被释放后消失。永远不会影响到 main 中 myValue 变量的“值”。

为了解决这个问题，我们可以用另一种方式分配内存以获取引用操作。我们将main中 myValue 变量的地址给到函数中，此函数接收地址后操作此地址达到修改 main 中 myValue 的“值”的目的。
...
在函数调用期间，不再在堆栈上复制该值，而是复制该值的地址。该函数现在通过本地指针变量引用同一块内存空间，并更改值。
...
new 关键字用于在内存中分配指定类型的值。内存分配后被清零。在调用new时无法进一步初始化内存。换句话说，使用new时，不能为指定类型的属性赋值。

如果要在分配值时指定值的内容，请使用复合文字。无论是否指定字段名称，它们都是可行的。
...
虽然使用 ChangeMyMapAddr 是没有必要的，但这个函数显示了如何在 main 中传递和使用 myMap 变量的地址进行操作。Go 的设计者再次确保传递 Map 变量的“值”可以毫无问题地执行。注意当我们想要访问 Map 时我们不能引用 myMapPointer 变量，这是因为 Go 编译器不允许我们直接通过指针变量访问 Map。取消了引用已经具有引用功能的数据类型。

我花时间写这篇文章的原因，是因为有时候你的变量的“值”包含什么，这一点可能会经常令人很困惑。如果你的变量的“值”是一个较大的值，并且你将该变量的“值”传递给函数，那么你将在堆栈上创建该变量的大副本。除非你有一个非常特殊的用例，否则你一定会希望将值的地址传递给函数以完成调用。

Map ,Slice 和 Channel 这些数据类型和普通的数据类型是不同的。你可以按值传递这些变量而无需担心。当我们将 map 变量传递给函数时，我们其实正在复制其中的地址而不是其中所有的数据。

最后我建议你阅读上面提到的 Effective Go 文档。自从我开始使用Go编程以来，我已多次阅读该文档。随着我的 Go 开发经验与日俱增，我还总是时不时回头温习之。总是有一种温故而知新的感觉！
