...
- 第一篇文章快速熟悉TDD
- 第二篇文章讨论数组及切片
- 第三篇文章教授类、方法、接口还有表驱动测试
- 第四篇文章展示如何处理异常及指针为什么有用
- 第五篇文章解释为什么及如何使用依赖注入
- 第六篇文章介绍并发相关
- 第七篇文章讲解使用mock工具的理由和方法

本章是关于基于select 的同步异步过程的介绍
##Select
[本章节中所有源码在此处](https://github.com/quii/learn-go-with-tests/tree/master/select)

首先我们要编写一个名为 `WebsiteRacer` 的函数，该函数将通过http GET方法对两个url的请求时间进行一个“赛跑”，如果两个请求在10秒内没有响应，即返回错误信息。
...
-  2、接下来，我们使用[http.Get]('https://golang.org/pkg/net/http/#Client.Get')尝试请求获取url的内容。这个方法将返回一个[http.Response]('https://golang.org/pkg/net/http/#Response"') 和一个错误值，但是目前为止我们不需要关心这个值。
- 3、`time.Since` 需要一个起始时间，并且会返回一个`time.Duration`的时间差。
一旦我们完成了这一步，我们只需比较持续时间，看哪个最快

###疑问
这个测试也许并不适用于你，因为我们使用的是真实的网站进行测试自己的逻辑。
对于使用HTTP这种很常用的测试，Go有一个标准库工具可以帮助我们测试。
...
事实证明，没有什么特别的魔法在这，这也是你如何使用go语言编写一个真正的HTTP服务。唯一的区别是我们将其包装成了`httptest.NewServer`，这也使得测试更加容易，因为它找到了一个开放的端口来监听请求，当你完成测试后可以将其关闭。

在我们的两个服务中，我们使其中一个满的休眠一小段时间，当我们得到返回信息的时候让它比另一个慢一点。两个服务后边都会返回给请求者一个OK的响应，使用`w.WriteHeader(http.StatusOK)`。
...
```
我们重构了名为`makeDelayedServer ` 的方法，用来构造我们的假服务。并将一些无聊的代码移除测试从而减少重复。
...
###进程同步
- go是十分擅长并发的，那我们为什么还要一个接一个的测试网站响应速度呢？我们可以同时测试两者。
