...
在今年的[Advent of Code](http://adventofcode.com/2017)中，有些问题需要不同的类型来读取输入。我最后每种技术都至少使用过一次，现在我对这些技术有所理解。我将我对这些技术的理解写在本文中。我列出的方法是按照我使用的顺序，并不是按照难度递减的顺序。
...
在 go 中像大多数低级语言和一些动态语言（例如Node）中一样，读取文件时返回一个字节流。不自动将读取内容转换为字符串有一个好处，可以避免因为昂贵的字符串分配而给 GC 带来的压力。

为了让这篇文章有一个简单的概念模型，我会使用 `string(arrayOfBytes)` 作为将字节数组转换成字符串的例子。不过一般来说不建议在生产环境使用这种方式进行转换。
...
1. 文件大小不超过内存。
2. 我们能够提前知道文件的大小，以便实例化一个足够大的缓冲区来保存它。

获得一个 `os.file` 对象的句柄，我们可以查询其大小并实例化一个字节类型切片。
...
3. 如果文件大小不是缓冲区大小的整数倍，则最后一次迭代只向缓存中添加余下的字节，因此需要读取 `buffer[:bytesread]`。在正常情况下，`bytesread`和缓存大小相同。
...
循环中的每个迭代，内部文件指针位置进行更新。当下一次读取时，数据从文件指针偏移开始，读取并返回缓冲区大小的数据。这个指针不是由编程语言创建的，而是操作系统创建的。在Linux上，这个指针是操作系统创建的文件描述符。所有读/读调用（分别在 Ruby/Go 中）被内部翻译成系统调用,并发送到内核，由内核管理这个指针。

### 并发读取文件块

如果我们想加快上面提到的对数据块的处理速度呢？一个方法就是使用多个 `go routine`!相对于顺序读取数据快，我们需要一个额外的操作就是要知道每个 `routine` 读取数据的偏移量。注意，`ReadAt` 函数和 `Read` 函数在缓存的大于剩余需要读取字节的时，处理方式略有不同。
...
这比以前任何方法都要复杂：
1. 我尝试创建一个特定的goroutine，这取决于文件大小和缓冲区大小（在我们的例子中是100）。
2. 我们需要一种方法来确保我们“等待”所有的goroutine运行完成。在这个例子中，我是使用WaitGroup。
3. 我们在goroutine运行完成时发送一个结束信号，而不是使用无限循环等待运行结束。我们使用defer调用wg.Done()，当goroutine运行到“return”时，wg.Done会被调用。

注意：总是要检查返回的字节数，并对输出缓冲区重新切片。

## 扫描文件

你可以一直使用Read()来读取文件，但有时你需要更多的便利。在Ruby中有一些经常用到的IO函数，比如each_line，each_char，each_codepoint等。我们可以使用Scanner类型和相关bufio包种提供函数来实现类似的功能。

bufio.Scanner类型实现了一个参数为“split”函数的函数，并基于此函数前进指针。例如，内置的bufio.ScanLines分割函数,在每次迭代中都会前进指针，直到下一个换行符。在每个步骤中，bufio.Scanner类型提供函数来获取在起始位置和结束位置之间的字节数组/字符串。例如：
```
file, err := os.Open("filetoread.txt")
if err != nil {
    fmt.Println(err)
...
bufio包包含几个基本预定义的拆分函数：
1. ScanLines （默认）
2. ScanWords
3. ScanRunes （在处理UTF-8编码时非常有用）
4. ScanBytes

所以，读取一个文件，并且按照单词分割并创立一个列表，可以使用以下代码：
```
file, err := os.Open("filetoread.txt")
if err != nil {
    fmt.Println(err)
...
```
ScanBytes分割函数将给出与我们之前使用Read()示例中相同的输出。两者之间的一个主要区别是每次我们都需要动态将数据追加到byte/string 的问题。这可以通过像预先初始化缓存的技术来规避，只在数据长度超出缓冲区时才增加缓冲区大小。使用上面的相同的例子：
```
file, err := os.Open("filetoread.txt")
if err != nil {
    fmt.Println(err)
...
所以我们显著减少了 slice “增长”操作，但是根据缓存大小和文件大小，我们可能会在缓存末尾有空缺，这是一个折衷的方案。

### 将长字符串分割成单词
bufio.NewScanner需要满足 io.Reader 接口的类型作为参数，这意味着它可以接受任何有 Read 方法的类型作为参数。标准库中字符串实用方法 strings.NewReader 函数返回一个具有 Reader 方法的类型。需要安单词读取时我们可以把两者结合起来：
...
用基本的 Read() 函数或 Scanner 类型手动解析CSV文件/字符串是比较繁琐的，因为上述分割函数 bufio.ScanWords 一个“单词”被定义为一组由空格分割的字符。读取单个字符，并记录缓冲区大小和位置（像词法分析/解析工作）需要太多的工作和操作。
