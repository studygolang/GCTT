...
在执行代码时，线程将变量（数据）存储在称为 stack 的内存区域中，这个区域会划分变量持有临时空间的地址。 堆栈是在编译时创建的，通常是固定大小，最好是1-2 MB。而线程的堆栈只能由该线程使用，不会与其他线程共享。堆是进程的属性，任何线程都可以使用它。堆是一个共享内存空间，其中一个线程的数据也可以被其他线程访问。
