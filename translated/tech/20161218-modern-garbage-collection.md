...
> 在更高的层次上，解决性能问题的一种方法是添加 GC 旋钮（knob），每个性能问题添加一个。然后，程序员可以旋转旋钮（knob）以搜索适合其应用的设置。不利的一面是，每年使用一个或两个新旋钮（knob），十年之后，你最终会得到 GC 旋钮（knob）特纳就业法案（应该的意思是：需要详细的文档描述说明什么情况下用哪一个 knob，以及什么情况下如何设置）。Go 不会走那条路。相反，我们只提供了一个叫做 GOGC 的旋钮（knob）。
>
...
<!-- 很别扭，找不到主语 -->
由于 Go 是一种具有值类型的相对普通的命令式语言，其内存访问模式可能与 C＃ 相当，其中世代假设当然成立，因此 .NET 使用世代收集器。
