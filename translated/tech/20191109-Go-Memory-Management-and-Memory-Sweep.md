...
将内存置 0 的过程 —— 就是把内存段中的所有位赋值为 0 —— 是在分配过程中即使执行的。
...
关于后台工作程序，当开始运行程序时，Go 将设置一个后台工作程序（只有清楚内存的内务），它将进入睡眠状态并等待跨度扫描：
...
我们也能看到这个后台工作程序总是在跟踪的循环期间出现来清理内存：

![](https://raw.githubusercontent.com/studygolang/gctt-images2/master/20191109-Go-Memory-Management-and-Memory-Sweep/08.png)

<p align="center">Background sweeper</p>
清理这些范围的第二种方式是即时执行。但是，由于这些跨度已经被分发到每一个处理器的本地缓存`mcache` 中，因此很难追踪首先清理哪些内存。这就是为什么 Go 首先将所有跨度移动到`mcentral`的原因。
...
即时扫描确保所有跨度在保存资源的过程中都会得到清理，并且不会阻止程序执行。
