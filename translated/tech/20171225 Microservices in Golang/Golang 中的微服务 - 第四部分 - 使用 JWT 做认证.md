...
照常，我写了一些注释解释了部分细节，不过这里的导言写的比较简单。Decode 方法，是将一个字符串 token 解析成一个 token 对象，验证下，如果有效返回一个 CustomClaims 对象。这样我们能通过这个 CustomClaims 对象获取到用户的元数据来确认该用户是否有效。
