...
FreeBSD 发明了 kqueue [ [论文](https://people.freebsd.org/~jlemon/papers/kqueue.pdf) , [文档](https://www.freebsd.org/cgi/man.cgi?query=kqueue&sektion=2) ]
...
* 代表对这个事件要执行操作的 `Flag`：在我们例子中，我们想要添加（EV_ADD）事件到 `kqueue`，比如说订阅事件，同时要启用（EV_ENABLE）它。
Flag 可以使用 `或` 这个位操作进行结合。
