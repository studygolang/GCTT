...
那如果应用需要和其他应用、主机或外部网络相互通信呢？您会如何在保持应用可移植性、服务发现、负载均衡、安全性、性能和可拓展性的同时，设计一个拥有适当连通性的网络呢？本文档将解决这些网络设计的问题，提供可用的工具以及通用的部署模式。文章不会指定或推荐物理网络的设计，但会给出一些兼顾应用的需求与物理网络条件约束的 Docker 网络设计方法。
