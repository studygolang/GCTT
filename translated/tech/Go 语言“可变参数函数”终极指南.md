...
虽然可变参数函数只是一种语法糖，但由它的函数签名——[类型推比（ type identity ）]()—— 与以切片作为参数的普通函数并不相同。
...
举例来说，Go 语言标准库中的 ``Printf`` 可变参数函数可以接受仍和类型的参数，其实现是通过将类型声明为一个空的接口类型（ interface type ）。如此你便可以使用空接口类型让你的函数接受类型和数量都不确定的参数。
...
你也许在其他面对对象编程语言中已经熟悉此设计模式，而它于2014年在 Go 语言中被 Rob Pike 再次推广。它与[访问者模式](https://en.wikipedia.org/wiki/Visitor_pattern)有些相似。
