...
你同时还可以参考下面的Go runtime包里的栈实现代码：
http://golang.org/src/pkg/runtime/stack.h
http://golang.org/src/pkg/runtime/stack.c

当你要使用递归的时候，你需要意识到栈内存是一直增加的，直到遇到你设置好的anchor为止，它的内存才开始下降。当我们说Go并没有优化递归操作时，我们需要承认一个事实，Go并没有尝试着去减少栈内存的增长这一操作。这个时候tail calls就登场了。
...
```
上面的这个递归函数需要传入一个整数作为参数，并return回一个整数。如果传入函数值是1，那么函数将会立马返回该值。这个if函数包含了锚点，并开始展开栈来完成任务。
...
3，递归函数在return前调用自己三次，当到达锚点的时候有3个扩展栈桢对应每一个调用操作。<br>
...
什么是tail call？它是如何优化递归函数的？ 接下来将创建一个带有tail call的递归函数，了解它是如何解决递归函数消耗大量栈内存问题的。
...
```
你能发现他们之间的不同吗？它与我们如何使用堆栈和计算结果有关。在这个实现中，锚点竟然返回了最终结果！（加强一下，反馈后删）。除了最终值，我们不再需要任何栈的返回值。
...
0037 (./main.go:26) RET ,
```
包含tail call的函数产生了更多的汇编代码。 但是，结果却非常相似。 事实上，从性能的角度来看，我们已经让事情变得更糟了。

Go没有针对我们实施的尾调用（tail call）进行任何优化。 我们仍然进行所有相同的栈操作和递归调用。 所以我猜Go目前没有针对递归进行优化。 这并不意味着我们不应该使用递归，我们只要知道Go语言中有这种特性即可。
...
```
它同样采用tail call实现。一旦锚点被击中，最终答它就会产生案，并将答案放入频道。它并不是进行递归调用，而是生成一个Goroutine，它提供了我们在tail call示例中推送到栈的相同状态。
