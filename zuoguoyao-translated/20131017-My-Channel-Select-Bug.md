...
<br>
我当时正在测试一个已经上线运行的项目的新功能，忽然代码表现得非常糟糕。我被所看到的震惊了，后来搞清楚了原因。
...
```
这份代码的功能是运行一项任务然后中止它，它允许操作系统申请提前终止程序。我喜欢尽可能的彻底关闭一个程序。
<br>
上述代码创建了一个绑定到操作系统的 `channel`，并且在终端窗口查找 `ctrl + c`，如果它被按下，那么  `Shutdown` 就会被设置为 `true`，并且程序跳转回 `select` 语句。
##第一个Bug
观察如下代码
```go
case <-func() chan struct{} {
...
```
该代码使用包变量来指示运行的 Go routine 在 `ctrl + c` 按下时关闭。每当我按下它时，代码都在工作，那么为什么会有 bug 呢？
首先让我们运行数据竞争检测：
```go
go build -race
./test
...
Found 1 data race(s)
```
我使用的 `Shutdown flag` 出现在了数据竞争检测器中，这是由于有两个 Go routine 在尝试用不安全的方法访问它。
我最初用不安全的方案访问该变量的原因是为了实用，然而它是错的。我认为由于该变量只在必要时用以关闭程序，所以我不关心脏读，但是万一脏读恰好出现在读写该变量的一瞬间呢？如果脏读出现，我可以在下次循环捕获它，看起来没损失对吧？为什么要像这样增加一个复杂的 channel，或者为代码加锁呢。

这就涉及到 Go 内存模型了。
[Go Memory Model](https://golang.org/ref/mem)
[Go语言中文网翻译 Go 内存模型](https://studygolang.com/articles/819)

Go 内存模型不保证 Go routine 读 Shutdown变量时会被 main routine看到，main routine 只写 Shutdown 变量一次并且不会被写回主内存，因为 main routine 永远不会去读 Shutdown 变量。
虽然这次不会出什么问题，但是随着 Go 语言编译器变得越来越复杂，有可能它会决定完全消除对 `Shutdown` 的写入。虽然这种行为被 Go 内存模型所允许，同样的，我们不希望代码不能通过数据竞争检测。即使是出于实用的的原因这也不是一个好的例子。
...
```
我喜欢用 if 语句来检查 Shutdown 是否被设置，这样我能在需要的时候使用它。这个解决方案吧 Shutdown 变量从 boolean 变成了 int32，并且用原子函数来存储读取。
在 main routine 如果 `ctrl + c` 被检测到，Shutdown 变量会安全的从 0 变成 1。在 LanuchProcessor Go routine 中，它会和 1 比较。如果条件为真 Go rontine 就会返回。

有时候确实很让人惊奇，一个如此简单的问题包含了几个小问题，在一开始事情有些层面你从来没有考虑过或者意识到。尤其是那些看起来正常工作的代码。
